#include <Keyboard.h>
#include <KeyboardLayout.h>
#include <EEPROM.h>
//define pins
#define ENCA 14 //clk
#define ENCB 16 //dt
#define ENCS 10
#define SDAP 2
#define SCLP 3
//display soft
#include <Arduino.h>
#include <U8g2lib.h>
#ifdef U8X8_HAVE_HW_SPI
#include <SPI.h>
#endif
#ifdef U8X8_HAVE_HW_I2C
#include <Wire.h>
#endif
U8G2_SSD1306_128X32_UNIVISION_F_SW_I2C u8g2(U8G2_R0,SCL,SDA,U8X8_PIN_NONE);
// debouncing variables
const unsigned long debounceTime = 30; // debounce time in ms
unsigned long lastDebounceTime = 0;
//EEPROM bootvars
int stat=EEPROM.read(0);
int cst=stat;
void setup() {
  Serial.begin(9600);
pinMode(ENCA, INPUT);
pinMode(ENCB, INPUT);
pinMode(ENCS, INPUT_PULLUP);
u8g2.begin();
lightshow(stat);
}
//global vars
bool recheck=false;
void loop() {
pulse();
pulseloop();
if(cst!=stat){
lightshow(stat);}
if(btncheck()){
  if(!recheck){
  cmdshell(stat); // TRY MOVING BTNCHECK OUT OF THE SWITCH INTO THE LOOP
  recheck=true;
  delay(250);
   }
}else
  recheck=false;
//if(cst!=stat)
//Serial.println(stat);
cst=stat;
}
int clk=0;
int lst=digitalRead(ENCA);
void pulse(){
  clk=digitalRead(ENCA);
  if(clk!=lst&&clk==1) {
    // debounce the signal
    if (millis() - lastDebounceTime > debounceTime) {
      lastDebounceTime = millis();    
      if(digitalRead(ENCB)!=clk)
        stat++;
      else
        stat--;
    }
  }
  lst=clk;
}
const int min=1; //lowest case number 
const int max=8; //highest case number

//this func loops stat to prevent over/under flow
void pulseloop(){
if(stat>max)
stat=min;
if(stat<min)
stat=max;
}

void lightshow(int input){
  switch(input){
  case 1: //intentionally left intact for reference
    u8g2.clearBuffer();					// clear the internal memory
    u8g2.setFont(u8g2_font_profont11_tr);	// choose a suitable font
    u8g2.drawStr(0,10,"Settings");	// write something to the internal memory
    u8g2.setFont(u8g2_font_open_iconic_embedded_2x_t);
    u8g2.drawStr(105,22,"H");
    u8g2.sendBuffer();					// transfer internal memory to the display
    delay(10);  
    break;
  case 2:
    
    break;
  case 3:
    
    break;
  case 4:
    
    break;
  }
}
void cmdshell(int input){
  switch(input){
  case 1: //left intact for reference
    Keyboard.press(KEY_LEFT_CTRL);
    delay(250);
    Keyboard.press(KEY_LEFT_GUI);
    delay(250);
    Keyboard.press(KEY_ESC);
    delay(1000);
    Keyboard.releaseAll();
    break;
  case 2:
    
    break;
  case 3:
    
    break;
  case 4:
    
    break;
   }
}
//checks if you click the encoder
/*
IF YOU WANT, you can move the EEPROM.update line into the loop
This will make the last function used save after turning the knob
rather than after using the function
*/
bool btncheck(){
if(digitalRead(ENCS)==LOW)
{
EEPROM.update(0,stat);
return true;}
return false;
}